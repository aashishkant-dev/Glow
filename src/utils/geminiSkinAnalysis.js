...
5. Look at three zones separately, the way an in-person consultation actually would — close, clinical-grade inspection, not a glance — and write one short, specific, plain-language note per zone covering whichever of these are actually visible: texture, pore size/density, oiliness or shine, tone evenness, any fine lines, redness, or areas of hyperpigmentation. Note left/right asymmetry between the two sides of the face if there genuinely is any (e.g. more visible on one cheek than the other) — real dermatological observations are rarely perfectly symmetric, and calling that out reads as a more genuine, closer look, not a templated one. Leave a zone's note as an empty string if there's truly nothing notable — never invent detail to fill the field:
