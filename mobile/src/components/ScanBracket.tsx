 ...
 * full outline box). Used by the result photo's tappable zone markers
 * (SkinZoneOverlay) so a scan reads as a deliberate scanning system rather
 * than a generic box drawn on a photo.
 */
...
interface Props {
  ...
  size?: number;
  thickness?: number;
  // A live camera overlay (see the vision-camera-live-detection branch)
  // pulses gently to read as "actively scanning"; static result markers
  // don't need the motion, but the prop exists here too so both stay on
